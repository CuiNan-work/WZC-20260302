...
result_ratio = 0.2      # 回传数据与输入数据的比率
P_uav = 0.5             # UAV下行发射功率(W)
...
class UAVEnv(gym.Env):
    def __init__(self, render_mode=None):
        ...
        # 回传时延(UAV计算完成后返回结果)
        self.users_backhaul_delay = [0] * num_users
        # 总时延
        self.total_time = 0
    ...
    def reset( self,*,seed: Optional[int] = None,return_info: bool = False,options: Optional[dict] = None,):
        ...
        self.users_backhaul_delay = [0] * num_users # 回传时延
    ...
    def step(self, action):
        ...
        info = {
            ...
            "backhaul_delay": np.array(self.users_backhaul_delay).copy(),
    ...
    def compute_UAV_RIS_GT_gain(self,):
        ...
        for m in range(num_uavs):
            for k in range(num_users):
                h_urg[m, k] = (h_rg[k].conj().T @ ris_matrix @ h_ur[m]).item() # 复数标量
    ...
    def backhaul_delay(self):

        backhaul_delay = [0] * num_users
        composite_channel = self.compute_Composite_channel()
    ...
    def compute_total_delay(self):
        self.total_time = np.sum(self.users_comp_delay) + np.sum(self.users_comm_delay) + np.sum(self.users_backhaul_delay)
    ...
    def compute_normalization_bounds(self):
        ...
        # 最坏回传延迟
        SNR_back_worst = P_uav * h_worst / P_n_W
        R_back_worst = (B_total / num_users) * np.log2(1 + SNR_back_worst)
        R_back_worst_Mbps = max(R_back_worst * 1e-6, 1e-6)
        D_backhaul_worst = (result_ratio * max_task_per_user) / R_back_worst_Mbps

        # 最坏计算延迟（所有任务由单个UAV顺序执行）
        D_comp_worst = (total_max_task * C) / F_max

        # 总的最坏延迟
        max_delay_offload = D_comm_worst + D_comp_worst + D_backhaul_worst
        ...
        # 最优回传延迟
        SNR_back_best = P_uav * h_best / P_n_W
        R_back_best = B_per_user * np.log2(1 + SNR_back_best)
        R_back_best_Mbps = R_back_best * 1e-6
        D_backhaul_best = (result_ratio * min_task_per_user) / R_back_best_Mbps

        # 完美负载均衡的计算延迟
        avg_load_per_uav = (np.mean([L_min, L_max]) * num_users / num_uavs)
        D_comp_best = (avg_load_per_uav * C) / F_max

        self.min_delay_theoretical = D_comm_best + D_comp_best + D_backhaul_best
...
class CustomPrintCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if step % self.print_freq == 0:
            ...
            backhaul_d = info["backhaul_delay"]
            ...
            for k in range(num_users):
                ...
                print(f"    GT{k:1d}: {mode:5s} | Comm: {comm_d[k]:6.4f}s | Comp: {comp_d[k]:6.4f}s | Backhaul: {backhaul_d[k]:6.4f}s | "f"Task: {task:.3f} Mbit | Rate: {rates[k]:6.2f} Mbps")
